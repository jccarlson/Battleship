	...
	public Point getNextShot() {
		...
		switch(nextShot) {
			case RANDOM:	do {
							...
							break;
			case NORTH:		prevShot.move(prevShot.x, prevShot.y - 1);
							break;
			case EAST:		prevShot.move(prevShot.x + 1, prevShot.y);
							break;
			case SOUTH:		prevShot.move(prevShot.x, prevShot.y + 1);
							break;
			case WEST:		prevShot.move(prevShot.x - 1, prevShot.y);
							break;
		}
		...
		if((e.getEvent() & BattleshipEvent.HIT) == BattleshipEvent.HIT) {
			...
			case NORTH:
				if((foundAt != null) && ((int)prevShot.distanceSq(foundAt)) > 1) {
					prevShot = (Point) foundAt.clone();
					if(prevShot.y < (dim.height - 1) && !isShot[prevShot.x][prevShot.y + 1]) {
						nextShot = SOUTH; break decideMiss;
					}
					nextShot = RANDOM; break decideMiss;
				}
				if(prevShot.x < (dim.width - 1) && !isShot[prevShot.x + 1][prevShot.y]) {
					nextShot = EAST; break decideMiss;
				}
			case EAST:
				if((foundAt != null) && ((int)prevShot.distanceSq(foundAt)) > 1) {
					prevShot = (Point) foundAt.clone();
					if(prevShot.x > 0 && !isShot[prevShot.x - 1][prevShot.y]) {
						nextShot = WEST; break decideMiss;
					}
					nextShot = RANDOM; break decideMiss;
				}
				if(prevShot.y < (dim.height - 1) && !isShot[prevShot.x][prevShot.y + 1]) {
			...
			case SOUTH:
				if((foundAt != null) && ((int)prevShot.distanceSq(foundAt)) > 1) {
					prevShot = (Point) foundAt.clone();
					if(prevShot.y > 0 && !isShot[prevShot.x][prevShot.y - 1]) {
						nextShot = NORTH; break decideMiss;
					}
					nextShot = RANDOM; break decideMiss;
				}
				if(prevShot.x > 0 && !isShot[prevShot.x - 1][prevShot.y]) {
