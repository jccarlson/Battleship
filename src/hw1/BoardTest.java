...
/**
 * A simple tester for the BattleshipBoard and Ship classes.
 * <p>
 * Implements a text-based (UTC-8 or UTC-16BE) UI for a game of Battleship. It
 * allows the user to select the size of the board, place ships deliberately or
 * randomly, and fire shots and sink ships, while displaying the board status.
 * Upon all ships being sunk, the board is displayed a final time, along with
		...
		while (!v) {
			...
			try {
				...
				if (n >= 5)
					...
					System.out.println("Please input an integer value >= 5. ");
		...
		if (manual) {
			for (int i = 0; i < fleet.length; i++) {
				...
				while (!validShip) {
					...
					while (!v) {
						System.out.print(
								"Enter the row of the " + fleet[i].getName() + " (size " + fleet[i].getSize() + "): ");
						...
						try {
							...
							if (sRow >= 1)
								...
								System.out.println("Please input an integer value >= 1. ");
						} catch (InputMismatchException e) {
							System.out
									.println("Caught: InputMismatchException -- Please input an integer value >= 0. ");
							cin.next();
						}
					}
					...
					v = false;
					while (!v) {
						System.out.print("Enter the column of the " + fleet[i].getName() + " (size "
								+ fleet[i].getSize() + "): ");
					...
					if (!validShip)
						System.out.println("Not a valid placement. Please try again.");
				}
			}
		} else {
			System.out.println();
		...
		if (!testBoard.lockBoard()) {
			...
			while (!testBoard.gameLost()) {
				...
				if (testBoard.fireShot(r, c)) {
					...
					System.out.println("Invalid shot: You have already shot there or it is out of bounds. Try again!");
			}
