...
/**
 * An Applet which plays a game of Battleship, using the boardGUI package for
 * visual elements and the boardAPI package for game logic. Sound is handled
 * with the sound package.
 * 
...
public class BattleshipApplet extends JApplet implements MouseListener, BattleshipListener {
	...
	public void init() {
		...
		guiBoard = new BoardPanel(logicalBoard.getSize(), 20,
				new ColorScheme(new Color(50, 50, 255), Color.RED, Color.WHITE, Color.DARK_GRAY, Color.BLACK));
	...
	/**
	 * Configure UI consisting of a play button, a board panel, a status panel,
	 * and a message Bar.
	 */
	 ...
	 * Prompts the user for confirmation if the game isn't over yet. Adds the
	 * visual elements to the new logicalBoard's listeners. Resets all the
	 * visual elements. Sets gameOver to false.
	 * 
	 * @param event
	 */
	...
	/**
	 * Gets the coordinates of a click, passes them to the guiBoard which maps
	 * them to a BoardSquare, then attempts to fire a shot at that square.
	 * 
	 * @param e
	 *            A MouseEvent which contains which button was pressed and the
	 *            coordinates.
	 */
	@Override
	...
	/**
	 * Processes the results of a shot fired at the logical board, and updates
	 * the GUI if needed.
	 * 
	 * @param e
	...
	public void shotFired(BattleshipEvent e) {
		...
		if (status == 'H') {
			...
			// if a ship was sunk append "You sunk the <shipName>!!!" to the
			// display message
		...
		if (status == 'M')
			displayMessage("SHOT FIRED! MISS!");

		// if the shot was invalid, say you can't shoot there.
		if (status == ' ')
			displayMessage("Can't shoot there...");
